...
export type ContractFormStore = {
  contract: Contract;
  setField: <K extends keyof Contract>(k: K, v: Contract[K]) => void;
  reset: (init: Contract) => void;
};

export function contractFormStoreFactory(init?: Contract) {
  return createStore<ContractFormStore>((set) => ({
    contract: init ?? initialContract,
    setField: <K extends keyof Contract>(k: K, v: Contract[K]) =>
      set(
